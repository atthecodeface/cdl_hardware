...
class c_cdl_file:
    ...
    def header(self):
        ...
        For source files, output a namespace so that the module in the
        source file is a namespace - and any local structures etc are
        deemed local to that module, as desired.

        Header files in CDL are part of the global namespace, so do
        not 'namespace' them.
        """
        if self.is_source_file:
            return r"/** \addtogroup %s **/namespace %s {"%(self.filename_leaf_root, self.filename_leaf_root)
            pass
        return ""
    #f footer
    def footer(self):
        ...
        Source files have to close the module's 'namespace'
        """
        if self.is_source_file:
            return "}"
            pass
    ...
    def parse_include(self, l, r):
        ...
        l = l[p+1:]
        if self.is_source_file:
            r += " namespace %s {"%(self.filename_leaf_root)
            pass
        self.parse_states.pop()
        return (l, r, None)
    #f parse_typedef
    ...
    def parse_module_header(self, l, r):
        (nl, nr, which) = self.find_first(l, r, ["{"]  + self.comment_start_strings )
        if self.verbose:print ">>>>>%s___%s___%s___%s___"%(l,nl,nr,which)
        (nl, nr, which, parsed) = self.parse_comment_start_string( nl, nr, which )
        if parsed: 
            return (nl, nr, which)
        if which == "{": return (nl, nr, ["module_body"])
        return ("", r+l, None)
    #f parse_module_body
    def parse_module_body(self, l, r):
        ...
        if code_block_match:
            self.pending_documentation='\section %s'%code_block_match.group(1)
            pass
