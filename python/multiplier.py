...
class multdiv:
    ...
    #f div_shift
    def div_shift(self):
        results = self.comb_new(a_in=0, b_in=0, mult4=1<<(self.a_shift&3), shf=self.a_shift>>2, ah_0=ah_0_zero, ah_1=ah_1_zero, al_1=al_1_zero, set_bit=False, bit_to_set=0, carry_chain=False, ar=a_reg_abs_a_in, br=b_reg_abs_b_in )
        (next_acc_l, next_acc_h, next_a_reg, next_b_reg, alu_l_carry) = results
        self.accumulator = next_acc_l | (next_acc_h<<32)
        #self.a_reg = next_a_reg
        #self.b_reg = next_b_reg
        if self.b_shift>=0:
            self.a_reg = (-(self.b_reg << self.b_shift)) & mask32
        print "%08x %08x %016x"%(self.a_reg, self.b_reg, self.accumulator)
        self.stage = self.b_shift - self.a_shift
        return self.stage<0

    #f div_step_new
    def div_step_new(self):
        results = self.comb_new(a_in=0, b_in=0, mult4=1, shf=0, ah_0=ah_0_acc, ah_1=ah_1_zero, al_1=al_1_acc, set_bit=(self.stage>=0), bit_to_set=self.stage, carry_chain=False, ar=a_reg_abs_a_in, br=b_reg_abs_b_in )
        (next_acc_l, next_acc_h, next_a_reg, next_b_reg, alu_l_carry) = results
        ...
        #self.a_reg = next_a_reg
        #self.b_reg = next_b_reg
        self.a_reg = (self.a_reg >> 1) | sign_bit
        self.stage = self.stage-1
        print "%08x %08x %016x"%(self.a_reg, self.b_reg, self.accumulator)
...
class multdiv:
    ...
    def div_step_new(self):
        ...
        #self.a_reg = next_a_reg
        #self.b_reg = next_b_reg
        #self.b_reg = (self.b_reg >> 1) | sign_bit
        self.stage = self.stage-1
        print "%08x %08x %016x"%(self.a_reg, self.b_reg, self.accumulator)
        return (self.stage<0) or (next_acc_l==0)
